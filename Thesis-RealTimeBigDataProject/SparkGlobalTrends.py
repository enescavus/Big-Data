###################################################################################################################
#  author : ENES ÇAVUŞ
#  subject : Python file for transforming Global Trends via PySpark
###################################################################################################################
...
#         df.show()
#         df.write.option('nullValue', None).saveAsTable(name='default.trends', format='hive', mode="append")

# this function controls RDD structures - creates a specific rdd type with unique column names and types
# transform RDD for parallelizing
...
def handle_rdd(rdd):                                                                                                    
    if not rdd.isEmpty():                                                                                               
        ...
        df = ss.createDataFrame(rdd, schema=schema)
        df.show()                                                                                                       
        df.write.saveAsTable(name='default.trends', format='hive', mode='append')             
# givin a name to app                                                                                                         
sc = SparkContext(appName="GlobalTrend")
ssc = StreamingContext(sc, 1) 
# Hadoop hive connections between spark and hive - local file paths                                                                                                   
ss = SparkSession.builder.appName("GlobalTrend").config("spark.sql.warehouse.dir", "/user/hive/warehouse").config("hive.metastore.uris", "thrift://localhost:9083").enableHiveSupport().getOrCreate()                                                                                                                                                                                                                         
ss.sparkContext.setLogLevel('WARN')  
# getting data from kafka stream DIRECTLY                                                                                                                        
getStream = KafkaUtils.createDirectStream(ssc, ['trends1'], {'metadata.broker.list': 'localhost:9092'})                       
